...
# TRAVEL TIMES #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-
# download shapefile and raster from MAP
shp <- malariaAtlas::getShp(ISO = "NGA", admin_level = "admin1")
plot(shp)
...
# PfPR #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-
# get MAP shapefiles for SSA admin1
PfPR <- malariaAtlas::getRaster(
  surface = 'Plasmodium falciparum PR2 - 10 version 2020',
  year = 2018,
  shp = shp)
malariaAtlas::autoplot_MAPraster(PfPR)
...
m <- svyglm(dpt3 ~ travel, DHSdesign, family=quasibinomial("identity")) # can't converge - maybe try categorizing travel

m <- svyglm(dpt3 ~ travel, DHSdesign, family=binomial("logit"))
cbind(tidy(m), confint(m))

m <- svyglm(itn ~ travel, DHSdesign, family=binomial("logit"))
cbind(tidy(m), confint(m))
