...
cost_effectiveness <- function(file){
  ...
  files <- list.files(path = "M:/Hillary/GF-RTSS-CE/03_output/HPC/", pattern = "casestudy_*", full.names = TRUE)
...
# save output
saveRDS(dalyoutput, './03_output/dalyoutput_draws_casestudy.rds')

# calculate cases / DALYs averted
output <- outcome_averted(dalyoutput)

# save output
saveRDS(output, './03_output/scenarios_draws_casestudy.rds')
